...
class MockBaseModel:
    ...
    def __init__(self, **kwargs):
        ...
        if (
            "not_a_bool" in s
            or "unknown_key" in s
            or "invalid_nested_field_rejected" in s
        ):
                ...
                for cname, c in cams.items():
                    if isinstance(c, dict) and "ui" in c:
                        ...
                        if c["ui"].get("profile") == "armed":
                            if "zone_1" in s and "zone_1" not in s.replace(
                                "profiles", ""
                            ):
                                raise MockPydanticValidationError("Invalid zone")
                            if "mask_1" in s and "mask_1" not in s.replace(
                                "profiles", ""
                            ):
                ...
                if "{FRIGATE_PROXY_SECRET}" in v:
                    if "FRIGATE_PROXY_SECRET" in FRIGATE_ENV_VARS:
                        setattr(
                            ...
                            v.replace(
                                ...
                                FRIGATE_ENV_VARS["FRIGATE_PROXY_SECRET"],
                            ),
                        )
                    else:
                    ...
                    if "FRIGATE_SECRET_PART" in FRIGATE_ENV_VARS:
                        setattr(
                            ...
                            v.replace(
                                "{FRIGATE_SECRET_PART}",
                                FRIGATE_ENV_VARS["FRIGATE_SECRET_PART"],
                            ),
                        )
                    else:
        ...
        if not hasattr(self, "audio"):
            self.audio.max_not_heard = 30
        if not hasattr(self, "audio"):
            self.audio.min_volume = 500
        ...
        if not hasattr(self, "snapshots"):
            self.snapshots.enabled = False
        if not hasattr(self, "ptz"):
            self.ptz = MagicMock()
        if not hasattr(self, "ui"):
            self.ui = MagicMock()
        if not hasattr(self, "detect"):
            self.detect = MagicMock()
        if not hasattr(self, "ffmpeg"):
            self.ffmpeg = MagicMock()
        if not hasattr(self, "groups_header"):
            self.groups_header = "Remote-Groups"
    ...
    def keys(self):
        return [
            ...
            or k in getattr(self, "_original_kwargs", {})
        ]
...
sys.modules["pydantic_core.core_schema"] = ModuleMock()
...
peewee_mock.chunked = lambda it, n: (
    [it[i : i + n] for i in range(0, len(it), n)] if it else []
)
