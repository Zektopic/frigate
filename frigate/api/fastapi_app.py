...
def create_fastapi_app(
    ...
    if frigate_config.auth.enabled:
        ...
        if isinstance(secret, str):
            if len(secret) >= 64 and re.fullmatch(r"[0-9a-fA-F]+", secret or ""):
                try:
                    key_bytes = bytes.fromhex(secret)
                    logger.debug(
                        "Derived JWT key from hex secret (len=%d)", len(secret)
                    )
                except ValueError:
                    key_bytes = hashlib.sha256(secret.encode("utf-8")).digest()
                    logger.debug(
                        "Derived JWT key via SHA-256 (hex decode failed, len=%d)",
                        len(secret),
                    )
            else:
                key_bytes = hashlib.sha256(secret.encode("utf-8")).digest()
                logger.debug(
                    "Derived JWT key via SHA-256 (non-hex or short secret, len=%d)",
                    len(secret),
                )
        elif isinstance(secret, (bytes, bytearray)):
            key_bytes = bytes(secret)
            logger.debug("Derived JWT key from raw bytes (len=%d)", len(key_bytes))
        else:
            key_bytes = hashlib.sha256(str(secret).encode("utf-8")).digest()
            logger.debug(
                "Derived JWT key via SHA-256 (non-string secret, len=%d)",
                len(key_bytes),
            )
