...
## Update on Path Validation Tests
- Identified that `test_util_path.py` was failing natively when running `python3 -m unittest discover -s frigate/test -p 'test_util_path.py'` due to a missing `pathvalidate` dependency.
- Mocking `pathvalidate.sanitize_filename` in `test_runner.py` proved insufficient, as the mock behaves differently from the real library, causing test assertions designed around the library's actual behavior to fail.
- Installing `pathvalidate` directly (`pip install pathvalidate`) resolves the `ModuleNotFoundError` and allows the 24 tests in `test_util_path.py` to pass perfectly without mocking.
- **Recommendation:** The fallback test runner should rely on installing pure-python lightweight dependencies (like `pathvalidate`) in the local virtual environment rather than mocking them, because testing sanitation logic against a naive mock defeats the purpose of the test.
