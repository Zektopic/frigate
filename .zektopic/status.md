...
## Backend Test Execution (2026-09-06 00:29:03)
- Attempted to run tests using docker (`make run_tests`), but encountered a buildkit overlayfs mount error (`invalid argument`), failing the docker build process.
- Executed local tests (`python3 test_runner.py`), resulting in numerous errors primarily related to mocking (89 failures, 200 errors).
  - Common errors include `PermissionError` for `/config/model_cache` creation, `AssertionError` for mocked functions returning `MagicMock` instead of expected types (like strings or integers), and `AttributeError` for Peewee models missing database operational methods.
- Rust tests executed successfully using `cargo test` in all subdirectories (`frigate-detector-rs`, `frigate-frame-rs`, `frigate-motion-rs`, `frigate-yolo-rs`), indicating standard and pixel pipelines are functioning correctly.
- Web tests were successful when isolated to the `src/` directory to avoid Playwright conflicts (`npm run test -- --run src/`).
